...
public class dialog : MonoBehaviour
{
        ...
        else
        {
            ...
            switch (_stage)
            {
                case "stage1_2":
                    if (SUI.stageclear >= 1)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 1 ? true : false);
                    }
                    break;
                case "stage2_1":
                    if (SUI.stageclear >= 2)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 2 ? true : false);
                    }
                    break;
                case "stage3_1":
                    if (SUI.stageclear >= 3)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 3 ? true : false);
                    }
                    break;
                case "stage4_1":
                    if (SUI.stageclear >= 4)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 4 ? true : false);
                    }
                    break;
                case "stage5_1":
                    if (SUI.stageclear >= 5)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 5 ? true : false);
                    }
                    break;
                case "stage6_1":
                    if (SUI.stageclear >= 6)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 6 ? true : false);
                    }
                    break;
                case "stage7_1":
                    if (SUI.stageclear >= 7)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 7 ? true : false);
                    }
                    break;
                case "stage8_1":
                    if (SUI.stageclear >= 8)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 8 ? true : false);
                    }
                    break;
                case "stage9_1":
                    if (SUI.stageclear >= 9)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 9 ? true : false);
                    }
                    break;
                case "stage10_1":
                    if (SUI.stageclear >= 10)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 10 ? true : false);
                    }
                    break;
                case "stage11_1":
                    if (SUI.stageclear >= 11)
                    {
                        SkipButton.SetActive(SUI.stageclear >= 11 ? true : false);
                    }
                    break;
    ...
    */
    ...
    public void startEff(GameObject Go)
    {
        RectTransform rectTransform = Go.GetComponent<RectTransform>();

        StartCoroutine(ShakeEff(rectTransform));
    }

    IEnumerator ShakeEff(RectTransform ret)
    {
        float _TIME = 0.0f;
        Vector2 _ORGUN_VEC = ret.anchoredPosition;

        while (_TIME < 1.0f)
        {
            ret.anchoredPosition = new Vector2(ret.anchoredPosition.x + (Random.Range(-2f,2f)), ret.anchoredPosition.y);
            _TIME += Time.deltaTime;
            Debug.Log(_TIME);
            yield return new WaitForSeconds(Time.deltaTime);
        }
        ret.anchoredPosition = _ORGUN_VEC;

        yield break;
    }
    public void startEff2(GameObject Go)
    {
        RectTransform rectTransform = Go.GetComponent<RectTransform>();

        StartCoroutine(ShakeEff2(rectTransform));
    }

    IEnumerator ShakeEff2(RectTransform ret)
    {
        float _TIME = 0.0f;
        Vector2 _ORGUN_VEC = ret.anchoredPosition;
